...
// Comprehensive SDK validation system
export function isValidRelayerSDK(
  ...
  // Check required configuration objects
  ...
  console.log("[RelayerSDKLoader] SDK validation passed");
  return true;
}
...
export function isFhevmWindowType(
  ...
  return isValidRelayerSDK(win.relayerSDK, trace);
}

// Enhanced property validation with detailed error logging
...
>(
  ...
  if (!obj || typeof obj !== "object") {
    trace?.(`RelayerSDKLoader: Object is null or not an object`);
    return false;
  }

  if (!(propertyName in obj)) {
    trace?.(`RelayerSDKLoader: Missing property ${String(propertyName)}`);
    return false;
  }
  ...
  if (value === null || value === undefined) {
    trace?.(`RelayerSDKLoader: Property ${String(propertyName)} is null or undefined`);
    return false;
  }

  if (typeof value !== propertyType) {
    trace?.(
      `RelayerSDKLoader: Property ${String(propertyName)} is ${typeof value}, expected ${propertyType}`
    );
    return false;
  }
